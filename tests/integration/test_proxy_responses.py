...
async def test_v1_responses_previous_response_followup_without_http_bridge_recovers_owner_from_request_logs(
    ...
    settings = await proxy_module.get_settings_cache().get()
    settings.upstream_stream_transport = upstream_transport
    ...
    assert persisted_log.upstream_transport == upstream_transport
    ...
    if upstream_transport == "http":
        # Stateless HTTP anchors cannot be revived from logs after local state
        # is lost; the public contract requests replay via stream_incomplete.
        assert second_response.status_code == 502
        assert second_response.json()["error"]["code"] == "stream_incomplete"
        assert selection_preferred_ids == [None]
    else:
...
@pytest.mark.asyncio
async def test_v1_responses_without_http_bridge_routes_oversized_explicit_websocket_upstream_over_http(
    async_client,
    monkeypatch,
):
