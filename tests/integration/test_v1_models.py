...
async def test_backend_codex_models_uses_bootstrap_upstream_metadata(async_client):
    ...
    for slug, expected_version in EXPECTED_BOOTSTRAP_MINIMAL_CLIENT_VERSIONS.items():
        ...
        expected_shell = "unified_exec" if slug == "gpt-6-astra" else "shell_command"
        assert entries[slug]["shell_type"] == expected_shell
    ...
    assert astra["apply_patch_tool_type"] == "freeform"
    assert astra["web_search_tool_type"] == "text_and_image"
    assert astra["supports_image_detail_original"] is True
    assert astra["multi_agent_version"] == "v2"
    assert astra["multi_agent_reasoning_effort"] == "xhigh"
    assert astra["experimental_supported_tools"] == ["send_user_message_async", "clock"]
    assert astra["node_repl_disabled"] is False
    assert astra["node_repl_auto_review_required"] is True
    assert astra["include_skills_usage_instructions"] is False
    assert astra["include_apps_usage_instructions"] is False
    assert astra["include_plugin_usage_instructions"] is False
    assert astra["effective_context_window_percent"] == 95
    assert astra["comp_hash"] == "3000"
    assert astra["default_reasoning_summary"] == "none"
    assert astra["truncation_policy"] == {"mode": "tokens", "limit": 10_000}
