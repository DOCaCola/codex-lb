...
- Client-facing WebSocket requests that exceed the upstream websocket budget after historical slimming fail locally with status `413` and `error.code = "payload_too_large"`. The official client retries the WebSocket stream and then switches that client session to HTTPS, allowing the same request to reach the upstream HTTP endpoint through codex-lb. HTTP requests retain status `400`, because they cannot recover through another transport switch. Current Codex clients default to 5 stream retries before the session-wide fallback.
