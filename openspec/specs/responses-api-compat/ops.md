...
- The downstream WebSocket ingress default follows `CODEX_LB_RESPONSES_BODY_LIMIT_BYTES` (128 MiB when unset; validated 32–512 MiB). This restart-required instance setting also bounds raw/decoded Responses and compact HTTP admission and expanded replay. Explicit `--ws-max-size` / `UVICORN_WS_MAX_SIZE` overrides only the listener. The budget applies to the decompressed message size; `permessage-deflate` stays negotiated on the client-facing socket.
- Create frames above the upstream WebSocket budget use HTTP SSE for that turn. The downstream WebSocket remains open, images and tool outputs remain intact, and smaller turns remain eligible for upstream WebSocket. An HTTP upstream 413 becomes a terminal `context_length_exceeded` event. Expanded bodies above the Responses HTTP budget are rejected before dispatch.
