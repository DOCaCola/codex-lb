...
- Select eligible Claude accounts with deterministic rendezvous affinity scoped to client key, conversation and model. This requires no process-local rotation state and survives worker changes. Filter source/model authorization before considering affinity. Account-bound continuation has a mandatory owner: unavailable or unauthorized owners produce an explicit error, not cross-account replay. Unbound portable requests may choose another eligible account before dispatch. No candidate is not permission to route an Anthropic model to ChatGPT.
